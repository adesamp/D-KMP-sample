...
struct CountriesListScreen: View {
    ...
    var body: some View {
        let countriesListState = CountriesListStateProvider(stateProvider : viewModel.stateProvider).getCountriesListState()
        let events = CountriesListEvents(events : viewModel.events)
